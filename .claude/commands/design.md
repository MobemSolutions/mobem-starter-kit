...
2. Mets à jour `src/app/globals.css` avec les tokens OKLCH
3. Synchronise `src/lib/constants/colors.ts`
4. Charge les polices dans `src/app/layout.tsx`
5. Adapte `src/components/layout/header.tsx` — nom réel, navigation réelle, CTA réel
6. Adapte `src/components/layout/footer.tsx` — NAP réel, liens réels

Puis : **STOP**. Indique de lancer `pnpm dev` pour vérifier visuellement les couleurs
